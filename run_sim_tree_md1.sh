...
BURN=2000
ITER=8000
MDSCEN=1 #missing data scenario 1=no missing data, 2=uniform [0,1] missing data, 3=10% missing data, 4=20% missing data, 5=30% missing data
...
for ((i=1; i<=2; i++)); do
    for ((j=1; j<=5; j++)); do
        ...
        if [ "$NST" != 0 ]; then
            ...
            #delete files
            rm ms_sim_data_paup_"$PBS_ARRAYID"_md_"$MDSCEN".nex ms_sim_data_rb_"$PBS_ARRAYID"_md_"$MDSCEN".tsv pars_paup_output"$PBS_ARRAYID"_md_"$MDSCEN".nex map_tree_"$PBS_ARRAYID"_md_"$MDSCEN".txt ml_tree_"$PBS_ARRAYID"_md_"$MDSCEN".txt mcmc_trees_"$PBS_ARRAYID"_*_md_"$MDSCEN".trees nst_"$PBS_ARRAYID"_md_"$MDSCEN".txt nj*_tree*_"$PBS_ARRAYID"_"$i"_"$j"_"$MDSCEN".new bal*_tree*_"$PBS_ARRAYID"_"$i"_"$j"_"$MDSCEN".new ml_tree_"$PBS_ARRAYID"_md_"$MDSCEN".new map_tree_"$PBS_ARRAYID"_md_"$MDSCEN".new pars_paup_output"$PBS_ARRAYID"_md_"$MDSCEN".new
        else
